...
//		}
		...
		public void run() 
		{
			...
			public void update() 
			{
				// Kiểm tra chuyển đổi từ ngày sang đêm
	            if (eManager.lighting.dayState == eManager.lighting.night) {
	                for (Entity obj : player.inventory) {
	                    if (obj instanceof OBJ_Tent) {
	                        OBJ_Tent tent = (OBJ_Tent) obj;
	                        if (tent.isProvidingInvisibility()) {
	                            tent.deactivateInvisibility();
	                            break;
	                        }
	                    }
	                }
